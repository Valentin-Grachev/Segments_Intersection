...
// Структура точек, сравнение которых идет приоритетно по ординате, затем по обсциссе
struct Point
{
	double x;
	double y;
...
// Структура точек, сравнение которых идет приоритетно по абсциссе, затем по ординате
struct PointX
{
	double x;
	double y;
...
// Проверка на то, лежат ли точки проверяемого отрезка по разные полуслоскоти образуемой секущим отрезком
bool PointsInDifferentHalfPlanes(Segment secant, Segment checkable)
{
	// Откладываем два новых вектора - от начала секущего отрезка в концы проверяемого отрезка
	...
	// Если точки проверяемого отрезка лежат по разные полуплоскости относительно секущего отрезка,
	// то векторные произведения между секущим вестором и созданными векторами должны иметь разный знак у координаты z
	...
	{
		// Хотя бы одна из точек отрезка совпала с секущим - возращаем true
		return true;
	}
